...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("lp.ingresaconvideos.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://lp.ingresaconvideos.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://lp.ingresaconvideos.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("lp.ingresaconvideos.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
